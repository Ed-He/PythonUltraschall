...
"""
Programmteil fuer Hallsensor
"""
# Zuordnen des Pins
GPIO_detected = 24
# Zuweisen der Ein- und Ausgaenge
GPIO.setup(GPIO_detected, GPIO.IN)


# Interrupt-Service-Routine fuer Hallsensor
def ISR(GPIO_detected):
    global client
    client.publish("201000000/machines/AMB-Motor/count", json.dumps({"count": 1}), qos=1)


# Hinzufuegen eines Interrupts
GPIO.add_event_detect(GPIO_detected, GPIO.RISING, callback=ISR, bouncetime=50)
...
def analysis():
    ...
    if (buffer < 40):  # Abfrage ob Objekt im Bereich
        abstand = buffer
...
def init_mqtt():
    ...
    client.will_set("201000000/machines/AMB-Motor/productionstatesU", json.dumps({"rule_Id": "rule_amb", "state": 11}),
                    qos=1)
...
def signal_term_handler(signal, frame):
    global client
    client.publish("201000000/machines/AMB-Motor/productionstatesU", json.dumps({"ruleId": "rule_amb", "state": 11}),
                   qos=1)
    client.disconnect()
    GPIO.cleanup()
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    # Abbrechen der Messung bei KeyboardInterrupt (Strg + C)
        ...
        GPIO.cleanup()
        client.publish("201000000/machines/AMB-Motor/productionstatesU",
                       json.dumps({"ruleId": "rule_amb", "state": 11}), qos=1)
        client.disconnect()
